...
class TestVideo:
    ...
    @allure.step("开始执行用例")
    @pytest.mark.parametrize("case_title, username, password, page_title", read_line("video.txt", end=1))
    def test_video_authentication(self, case_title, username, password, page_title):
        try:
    ...
    @allure.title("{case_title}")
    @allure.step("开始执行用例")
    @pytest.mark.parametrize("case_title, username, password, page_title", read_line("video.txt", 1))
    def test_video_agreement(self, case_title, username, password, page_title):
        try:
