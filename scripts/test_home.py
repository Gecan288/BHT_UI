...
class TestHome():
    ...
    @allure.title("验证首页提示")
    @allure.step("开始执行用例")
    @pytest.mark.parametrize("username, password, home_toast, page_title", read_line("home.txt", end=1))
    def test_not_login(self, username, password, home_toast, page_title):
        try:
    ...
    @allure.story("用户登录")
    @allure.title("验证员工认证跳转和钱包跳转")
    @allure.step("开始执行用例")
    @pytest.mark.parametrize("username, password, page_title, wallet_title", read_line("home.txt", 1))
    def test_user_login(self, username, password, page_title, wallet_title):
        try:
